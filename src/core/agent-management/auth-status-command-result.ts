...
const EXPLICIT_AUTH_STATUS_PATTERN =
  /\b(?:logged\s+in\s+as|authenticated\s+as|authenticated\b\s*[:=]\s*(?:true|false|yes|no|1|0)|(?:auth(?:entication)?\s+)?status\b\s*[:=]\s*(?:authenticated|unauthenticated|logged[\s_-]?in|logged[\s_-]?out)|not\s+logged\s+in|logged\s+out|not\s+authenticated)\b|^(?:authenticated|unauthenticated|logged[\s_-]?in|logged[\s_-]?out)$/im;
