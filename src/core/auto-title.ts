...
export const generateSessionTitle = (messages: Message[]): string | null => {
  ...
  if (firstLine.length <= LIMIT.TITLE_MAX_LENGTH) return firstLine;
  return `${firstLine.slice(0, LIMIT.TITLE_MAX_LENGTH - 3)}...`;
};
...
export const buildTitlePrompt = (messages: Message[]): string => {
  ...
  for (const message of messages) {
    if (chars >= LIMIT.TITLE_CONTEXT_CHARS) break;
    for (const block of message.content) {
      if (block.type === CONTENT_BLOCK_TYPE.TEXT && "text" in block) {
        const remaining = LIMIT.TITLE_CONTEXT_CHARS - chars;
  ...
  return `Generate a short, descriptive title (max ${LIMIT.TITLE_MAX_LENGTH} characters) for this conversation:\n\n${contextParts.join("\n")}`;
};
