...
- Logging via `src/shared/utils/logger.utils.ts` (`createClassLogger`); add helpful context. dont use `console.log` unless its for `__tests__`
- Environment via `src/shared/utils/env.utils.ts` (`Env`); never read `process.env` directly. dont use `process.env` unless its for tests.
...
- Add a timestamped entry to `scratchpad/journal.md` (milestone + task notes, learnings, important meory for failure/restart states).
