...
| Prompt spawn overhead target | Cursor session flow integration samples prompt durations and enforces p95 threshold | `__tests__/integration/core/cursor-session-flow.integration.test.ts` (`PROMPT_OVERHEAD_P95_MAX_MS = 500`, `PROMPT_SAMPLE_COUNT = 5`) |
