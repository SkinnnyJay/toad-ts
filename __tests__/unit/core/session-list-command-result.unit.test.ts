...
describe("session-list-command-result", () => {
  ...
  it("treats no-chats status output as explicit empty result", () => {
    const parsed = parseSessionListCommandResult({
      stdout: "No chats found.",
      stderr: "warning: historical cache session-noise-id",
      exitCode: 0,
    });
  ...
  it("treats no-conversations status output as explicit empty result", () => {
    const parsed = parseSessionListCommandResult({
      stdout: "No conversations found.",
      stderr: "warning: old cache entry session-noise-id",
      exitCode: 0,
    });
