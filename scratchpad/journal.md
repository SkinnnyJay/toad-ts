...
# 2026-02-14 (B235 ancillary-api protected-variant parity)

### Summary
- Expanded protected ancillary known-API integration coverage for
  trailing/query/hash variants under auth-before-method ordering.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in protected API method-semantics integration, added explicit
    unauthenticated `POST` assertions for `/api/agents`, `/api/events`,
    `/api/files/search` across trailing/query/hash variants, in addition to
    existing base/double-trailing variants, expecting:
    - `401` + `www-authenticate: Bearer`
    - `AUTHORIZATION_REQUIRED` payload.
  - added matching authenticated `POST` assertions for those variants
    expecting `405 METHOD_NOT_ALLOWED`.
  - added explicit unauthenticated/authenticated parity assertions for
    unsupported `GET` on `/api/tui/append-prompt` and
    `/api/tui/submit-prompt` across trailing/query/hash variants.
...
# 2026-02-14 (B234 ancillary-api padded-path parity)

### Summary
- Expanded ancillary known-API classifier parity coverage for
  whitespace-padded pathname variants across API and server classifiers.
- Updated:
  - `__tests__/unit/server/api-routes.unit.test.ts`
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added API classifier assertions proving ancillary known routes preserve
    `MATCH` and `METHOD_NOT_ALLOWED` outcomes when pathname inputs are
    whitespace-padded and include normalized suffix forms.
  - added server-route classifier assertions proving matching top-level
    `API_MATCH` and API-scoped `METHOD_NOT_ALLOWED` outcomes for the same
...
### Summary
- Expanded `/api/sessions` session-collection coverage across API/server
  classifiers and protected integration flows.
- Updated:
  - `__tests__/unit/server/api-routes.unit.test.ts`
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - added API classifier assertions for `/api/sessions` unsupported-method
    semantics across base/trailing/query/hash/double variants (plain+padded).
  - added server classifier assertions for same semantics and API handler
    attribution.
  - expanded protected API integration to include `/api/sessions` variants with
    auth-before-method ordering:
    - unauthenticated `POST` => `401` + `Bearer` + `AUTHORIZATION_REQUIRED`
    - authenticated `POST` => `405 METHOD_NOT_ALLOWED`.
...
- Expanded known-route double-trailing coverage across API/core/server
  classifiers and headless-server integration semantics.
- Updated:
  - `__tests__/unit/server/api-routes.unit.test.ts`
  - `__tests__/unit/server/core-route-classifier.unit.test.ts`
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  ...
  - `PLAN3.md`
- Changes:
  - added API classifier unit coverage for known-route double-trailing variants
    (`//`, `//?query`, `//#hash`) on config/execute/session/messages paths.
  - added core/server classifier unit coverage proving known core routes keep
    stable method semantics under double-trailing normalization.
  - expanded integration coverage to lock end-to-end semantics for known API
    and non-API double-trailing variants, including protected route auth-before-
    method behavior for prompt/messages.

### Validation
- Targeted:
  - `npx vitest run __tests__/unit/server/api-routes.unit.test.ts __tests__/unit/server/core-route-classifier.unit.test.ts __tests__/unit/server/server-route-classifier.unit.test.ts __tests__/integration/server/headless-server.integration.test.ts __tests__/unit/server/pathname-normalization.unit.test.ts __tests__/unit/server/session-route-path.unit.test.ts` ✅
- Full gates (equivalent commands; bun/bunx unavailable in this shell):
  - `bun run lint` ❌ (`bun: command not found`)
  - `bun run typecheck` ❌ (`bun: command not found`)
  - `bun run test` ❌ (`bun: command not found`)
...
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added assertions proving all of the following normalize to `/sessions`:
    - `/sessions//`
    - `/sessions//?scope=all`
    - `/sessions//#summary`
    - ` /sessions// `
...
# 2026-02-14 (B223 padded blank-session base method parity)

### Summary
- Expanded classifier unit coverage for whitespace-padded blank-session
  double-trailing `/sessions//...` base variants to lock normalized method
  semantics in core/server classifiers.
- Updated:
  - `__tests__/unit/server/core-route-classifier.unit.test.ts`
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added core-route classifier assertions for whitespace-padded
    `/sessions//`, `/sessions//?scope=all`, `/sessions//#summary`:
    - GET => `METHOD_NOT_ALLOWED`
    - POST => `UNHANDLED`
  - added server-route classifier assertions for same whitespace-padded
    variants with matching semantics and `CORE_ROUTE_CLASSIFIER` attribution.

### Validation
- Targeted:
  - `npx vitest run __tests__/unit/server/core-route-classifier.unit.test.ts __tests__/unit/server/server-route-classifier.unit.test.ts __tests__/unit/server/session-route-path.unit.test.ts __tests__/integration/server/headless-server.integration.test.ts` ✅
- Full gates (equivalent commands; bun/bunx unavailable in this shell):
  - `bun run lint` ❌ (`bun: command not found`)
  - `bun run typecheck` ❌ (`bun: command not found`)
  - `bun run test` ❌ (`bun: command not found`)
...
- Expanded blank-session base variant coverage to lock how double-trailing
  `/sessions//...` forms normalize and classify across parser/core/server
  layers.
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added session-route parser assertions confirming whitespace-padded
    blank-session base forms return `null`:
    - ` /sessions// `
    - ` /sessions//?scope=all `
    - ` /sessions//#summary `
  - added core-route classifier assertions confirming blank-session base
    double-trailing variants normalize to `/sessions` semantics:
    - GET variants => `METHOD_NOT_ALLOWED`
    - POST variants => `UNHANDLED`
  - added server-route classifier assertions confirming same behavior with
    `CORE_ROUTE_CLASSIFIER` attribution.
...
# 2026-02-14 (B220 unknown-route method-normalization parity)
...
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added api-routes assertions showing normalized methods still classify
    unknown/malformed API paths as `NOT_FOUND`:
    - `classifyApiRoute(" get ", "/api/does-not-exist")`
    - `classifyApiRoute("post", "/api//config")`
    - `classifyApiRoute(" delete ", "/api/does-not-exist")`, etc.
  - added core-route assertions showing normalized methods classify
    unknown/malformed core/session paths as `UNHANDLED`.
  - added server-route assertions showing normalized methods preserve
    scope-specific unhandled routing:
    - unknown/malformed API paths => `UNHANDLED` / `API_ROUTE_CLASSIFIER`
    - unknown/malformed core/session paths => `UNHANDLED` /
...
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added core-route classifier assertions (`UNHANDLED`) for whitespace-padded:
    - ` /unknown-endpoint//?scope=all ` (GET)
    - ` /unknown-endpoint//#summary ` (POST)
    - ` /sessions/session-1//#latest ` (GET)
    - ` /sessions/session-1//?view=full ` (POST)
    - ` /sessions//prompt//#summary ` (GET)
    - ` /sessions//messages//?tail=1 ` (POST)
  - added server-route classifier assertions for same whitespace-padded
    variants to ensure `UNHANDLED` with `CORE_ROUTE_CLASSIFIER`.
...
# 2026-02-14 (B215 unknown-api whitespace padded parity)
...
- Changes:
  - added api-routes classifier assertions (`NOT_FOUND`) for whitespace-padded:
    ...
    with explicit GET/POST parity.
  - added server-route classifier assertions (`UNHANDLED` with
    `API_ROUTE_CLASSIFIER`) for whitespace-padded:
    - ` /api/unknown/ `
    - ` /api/unknown// `
...
- Changes:
  ...
  - added explicit authenticated POST assertions (`404` + `NOT_FOUND`) for the
    same route set, confirming auth-before-not-found ordering stability.
...
# 2026-02-14 (B211 api-unknown segment parity hardening)
...
- Changes:
  - added headless-server integration unauthenticated/authenticated POST parity
    ...
    - `/api/unknown?scope=all`
    - `/api/unknown#summary`
  - added api-routes classifier POST/GET parity assertions for unknown API
    path forms:
    - `/api/does-not-exist`
    - ` /api/does-not-exist `
    - `/api/does-not-exist?view=compact`
    ...
    - `/api/does-not-exist/?view=compact`
    - `/api/does-not-exist/#summary`
  - added server-route classifier POST parity assertion for `/api/unknown`.
...
# 2026-02-14 (B210 api-root method parity hardening)
...
- Changes:
  - added headless-server integration unauthenticated/authenticated POST parity
    assertions for API-root unknown variants:
    - `/api`
    - `/api?scope=all`
    - `/api#summary`
    - `/api/`
    ...
    - `/api//?scope=all`
    - `/api//#summary`
  - added api-routes classifier POST/GET parity assertions for the same API-root
    variant set, locking consistent `NOT_FOUND` classification.
  - added server-route classifier POST/GET parity assertions for the same
    API-root variant set, locking consistent API classifier ownership.
...
# 2026-02-14 (B209 unknown single-segment integration parity)
...
- Changes:
  - added explicit unauthenticated POST assertions (`401` + `Bearer` +
    `AUTHORIZATION_REQUIRED`) for unknown single-segment routes:
    - `/unknown`
    - `/unknown/`
    - `/unknown?scope=all`
    - `/unknown/?scope=all`
...
# 2026-02-14 (B208 unknown single-segment trailing parity)
...
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added core-route classifier unknown single-segment POST/GET parity
    assertions for:
    - `/unknown/`
    - `/unknown/?scope=all`
    - `/unknown/#summary`
    ...
    - `/unknown//?scope=all`
    - `/unknown//#summary`
  - added server-route classifier dedicated unknown single-segment
    trailing/double-trailing POST/GET parity assertions for the same path set.
...
- Changes:
  - added explicit unauthenticated POST assertions (`401` + `Bearer` +
    `AUTHORIZATION_REQUIRED`) for missing-action session routes:
    - `/sessions/session-1`
    - `/sessions/session-1?scope=all`
    - `/sessions/session-1/`
    - `/sessions/session-1/?scope=all`
    ...
    - `/sessions/session-1//?scope=all`
    - `/sessions/session-1//#summary`
  - added explicit authenticated POST assertions (`404` +
    `UNKNOWN_ENDPOINT`) for the same route set, confirming auth-before-not-found
    ordering remains stable under POST parity.
...
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added core-route classifier GET assertion for:
    - `/sessions//prompt/?tail=1`
    locking `UNHANDLED` parity for malformed prompt trailing-query form.
  - added server-route classifier POST/GET assertions for:
    - `/sessions//prompt/?tail=1`
    locking core classifier ownership parity.
...
- Changes:
  - added explicit null-parse assertions for:
    ...
    - `/sessions//?scope=all`
    - `/sessions//#summary`
    alongside existing blank-session malformed suffix coverage.
...
# 2026-02-14 (B197 blank-session trailing-suffix parse-route parity)
...
- Changes:
  - added explicit null-parse assertions for:
    ...
    - `/sessions//messages/?tail=1`
    - `/sessions//messages/#summary`
    alongside existing malformed suffix coverage.
...
- Changes:
  - added unauthenticated/authenticated GET assertions for:
    ...
    - `/sessions//prompt/?tail=1`
    alongside existing POST assertions, locking auth-first unknown-route
    semantics for blank-session prompt query malformed paths under password
    protection.
  - added core/server classifier GET assertions for:
    - `/sessions//prompt?tail=1`
    - `/sessions//prompt//?tail=1`
    locking canonical core `UNHANDLED` ownership parity.
...
# 2026-02-14 (B193 blank-session prompt base/trailing get-method parity)
...
- Changes:
  - added unauthenticated/authenticated GET assertions for:
    ...
    - `/sessions//prompt/`
    alongside existing POST assertions, locking auth-first unknown-route
    semantics for blank-session prompt malformed paths under password
    protection.
  - added core/server classifier GET assertions for the same malformed base
    and trailing prompt variants, locking canonical core `UNHANDLED`
    ownership parity.
...
# 2026-02-14 (B192 malformed-api base/trailing get-method parity)

### Summary
- Expanded unknown-route auth-order integration plus API/server classifier
  coverage for malformed API and API-session base/trailing variants with
  explicit GET-method parity.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `__tests__/unit/server/api-routes.unit.test.ts`
...
- Changes:
  ...
  - added API/server classifier GET assertions for the same malformed base
    and trailing variants, locking canonical API-scoped `NOT_FOUND`/
    `UNHANDLED` ownership parity.
...
- Changes:
  - added unauthenticated/authenticated GET assertions for:
    ...
    unknown-route semantics under password protection.
  - added API/server classifier GET assertions for the same malformed direct
    hash and trailing-hash variants, locking canonical API-scoped
    `NOT_FOUND`/`UNHANDLED` ownership parity.
...
# 2026-02-14 (B189 malformed-api double-trailing-hash get-method parity)
...
- Changes:
  - added unauthenticated/authenticated GET assertions for:
    ...
    alongside existing POST assertions, locking auth-first malformed API
    unknown-route semantics under password protection.
  - added API/server classifier GET assertions for the same malformed
    double-trailing-hash variants, locking canonical API-scoped
    `NOT_FOUND`/`UNHANDLED` ownership parity.
...
- Changes:
  - added unauthenticated/authenticated GET assertions for:
    - `/api//config//?scope=all`
    alongside existing POST variant assertions, locking auth-first malformed
    API unknown-route semantics under password protection.
...
# 2026-02-14 (B186 malformed-api-session double-trailing-query method parity)
...
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added `classifyApiRoute("GET", "/api/sessions//messages//?scope=all")`
    assertion locking API-scoped `NOT_FOUND` parity.
  - added `classifyServerRoute(GET, "/api/sessions//messages//?scope=all")`
    assertion locking API-scoped `UNHANDLED` parity with
    `API_ROUTE_CLASSIFIER` ownership.
...
# 2026-02-14 (B185 malformed-api-session double-trailing-query parity)
...
- Changes:
  - added unauthenticated/authenticated integration assertions for:
    - `/api/sessions//messages//?scope=all`
    locking auth-first malformed API-session unknown-route semantics under
    password protection for double-trailing-query variants.
  - added API/server classifier unit assertions for:
    - `/api/sessions//messages//?scope=all`
    locking canonical API-scoped not-found/unhandled ownership parity.
...
# 2026-02-14 (B184 malformed-api double-trailing-query parity)
...
- Changes:
  - added unauthenticated/authenticated integration assertions for:
    - `/api//config//?scope=all`
    locking auth-first malformed API unknown-route semantics under password
    protection for double-trailing-query variants.
  - added API/server classifier unit assertions for:
    - `/api//config//?scope=all`
    locking canonical API-scoped not-found/unhandled ownership parity.
...
# 2026-02-14 (B183 api-root double-trailing unknown parity)
...
- Changes:
  - added unauthenticated/authenticated integration assertions for:
    ...
    - `/api//?scope=all`
    - `/api//#summary`
    locking auth-first API-root unknown-route semantics under password
    protection for double-trailing variants.
  - added API/server classifier unit assertions for the same API-root
    double-trailing base/query/hash variants, locking canonical
    API-scoped not-found/unhandled ownership.
...
- Changes:
  - added unauthenticated/authenticated integration assertions for:
    - `/sessions//prompt//?tail=1`
    - `/sessions//messages//?tail=1`
    locking auth-first blank-session unknown-route semantics under password
    protection for double-trailing-query variants.
  - added core/server classifier unit assertions for the same
    double-trailing-query prompt/messages variants, locking canonical
    core-unhandled ownership.
  - added session-route-path unit assertions locking null parsing for the same
    blank-session double-trailing-query malformed variants.
...
# 2026-02-14 (B180 blank-session double-trailing unknown parity)
...
- Changes:
  - added unauthenticated/authenticated integration assertions for:
    ...
    locking auth-first blank-session unknown-route semantics under password
    protection.
  - added core/server classifier unit assertions for the same double-trailing
    prompt/messages variants, locking canonical core-unhandled ownership.
  - added session-route-path unit assertions locking null parsing for the same
    blank-session double-trailing malformed variants.
...
# 2026-02-14 (B179 malformed-api-session double-trailing unknown parity)
...
- Changes:
  - added unauthenticated/authenticated integration assertions for:
    - `/api/sessions//messages//`
    - `/api/sessions//messages//#summary`
    locking auth-first malformed unknown-route semantics under password
    protection.
  - added API/server classifier unit assertions for:
    - `/api/sessions//messages//`
    - `/api/sessions//messages//?scope=all`
    - `/api/sessions//messages//#summary`
    locking canonical API-scoped unhandled/not-found classifier parity.
...
# 2026-02-14 (B178 malformed-api double-trailing unknown parity)
...
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added classifier assertions for:
    - `/unknown-endpoint//`
    - `/unknown-endpoint//?scope=all`
  - locked canonical unhandled classification semantics:
    - `classifyCoreRoute` -> `UNHANDLED`
    - `classifyServerRoute` -> `UNHANDLED` with
...
## 2026-02-14 (B173 parsing-log suffix-only root normalization coverage)

### Summary
- Expanded request-error-normalization unit coverage for parsing-log
  normalization of suffix-only request paths.
- Updated:
  - `__tests__/unit/server/request-error-normalization.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added `logRequestParsingFailure` assertion for suffix-only request path
    (`#summary`) to lock normalized logged pathname as root (`/`) while
    preserving normalized handler/method metadata.
...
### Summary
- Expanded malformed API-session trailing-query classifier parity unit coverage
  across API and server route classifier suites.
- Updated:
  - `__tests__/unit/server/api-routes.unit.test.ts`
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - `api-routes`:
    - added malformed API-session trailing-query assertion
      (`/api/sessions//messages/?scope=all`) as API-scoped `NOT_FOUND`.
  - `server-route-classifier`:
    - added malformed API-session trailing-query assertion
      (`/api/sessions//messages/?scope=all`) as API-scoped `UNHANDLED` with
      `API_ROUTE_CLASSIFIER` ownership.
...
- Expanded core/server route classifier unit coverage for unknown-route hash
  parity and malformed API-session hash ownership.
- Updated:
  - `__tests__/unit/server/core-route-classifier.unit.test.ts`
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - `core-route-classifier`:
    - added unhandled assertions for `/unknown-endpoint?scope=all` and
      `/unknown-endpoint#summary`.
  - `server-route-classifier`:
    - added API-scoped unhandled assertions for malformed API-session hash and
      trailing-hash forms:
      `/api/sessions//messages#summary`,
      `/api/sessions//messages/#summary`.
...
- Full gates (equivalent commands; bun/bunx unavailable in this shell):
  ...
  - `npx biome check . && npx eslint .` ✅
  - `npx tsc --noEmit` ✅
  - `npx vitest run` ✅ (initial run hit a transient integration timeout; rerun passed)
  - `npx tsup` ✅
  - `npx tsx scripts/check-magic-literals.ts --strict` ✅

## 2026-02-14 (B165 request-url host candidate fallback hardening)

### Summary
- Expanded `parseRequestUrl` unit coverage for host candidate fallback
  semantics across additional invalid-host metadata forms.
- Updated:
  - `__tests__/unit/server/request-url.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added host-candidate fallback assertions where the first candidate is
    invalid due to:
    - hash metadata (`example.com#summary`)
    - userinfo (`user@example.com`)
    with later valid candidate selection.
  - added all-invalid candidate assertion:
    - `"%, example.com#summary"` returns `null`.
...
- Expanded password-protected unknown-route auth-before-not-found integration
  coverage for direct-query (non-trailing) variants across core, malformed
  API, malformed API session, missing-action session, and blank-prompt paths.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in
    `applies auth checks before not-found semantics on unknown routes`,
    added unauthenticated/authenticated direct-query assertions for:
    - `/unknown-endpoint?scope=all`
    - `/api//config?scope=all`
    - `/api/sessions//messages?scope=all`
    - `/sessions/session-1?scope=all`
    - `/sessions//prompt?tail=1`
    locking canonical auth-first `401` + challenge and authenticated `404`
    (`NOT_FOUND`/`UNKNOWN_ENDPOINT`) semantics.
...
## 2026-02-14 (B160 unknown-route blank-session hash auth-order coverage)

### Summary
- Expanded password-protected unknown-route auth-before-not-found integration
  coverage for blank-session prompt/messages hash-bearing variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in
    ...
    - `/sessions//messages#summary`
    - `/sessions//messages/#summary`
    locking canonical auth-first unauthenticated `401` + challenge and
    authenticated `404` (`UNKNOWN_ENDPOINT`) semantics.
...
## 2026-02-14 (B159 unknown-route malformed/missing-action hash auth-order coverage)

### Summary
- Expanded password-protected unknown-route auth-before-not-found integration
  coverage for malformed API session and missing-action session hash-bearing
  variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in
    ...
    - `/sessions/session-1/#summary`
    locking canonical auth-first unauthenticated `401` + challenge and
    authenticated `404` (`NOT_FOUND`/`UNKNOWN_ENDPOINT`) semantics.
...
### Summary
- Expanded password-protected non-API auth-order integration coverage for
  hash and trailing-hash variants on sessions/prompt/messages method guards.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in
    ...
    - `/sessions/session-1/prompt#summary`,
      `/sessions/session-1/prompt/#summary`
    - `/sessions/session-1/messages#summary`,
      `/sessions/session-1/messages/#summary`
    locking canonical pre-auth `401` + challenge and authenticated `405`.
...
### Summary
- Expanded password-protected API auth-order integration coverage for
  hash/trailing-hash unsupported-method variants on config and execute routes.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in
    `applies auth checks before method-not-allowed semantics on api routes`,
    added unauthenticated and authenticated assertions for:
    - `/api/config#summary`
    - `/api/config/#summary`
    - `/api/tui/execute-command#summary`
    - `/api/tui/execute-command/#summary`
    locking pre-auth `401` and post-auth `405` ordering.
...
## 2026-02-14 (B152 integration api hash method coverage)

### Summary
- Expanded headless-server integration coverage for known API
  method-not-allowed semantics on hash-bearing paths.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
...
### Summary
- Expanded session-route-path unit coverage for combined trailing-slash +
  query/hash route forms.
- Updated:
  - `__tests__/unit/server/session-route-path.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added assertions for:
    - `/sessions/session-1/prompt/#latest`
      -> `{ sessionId: "session-1", action: "prompt" }`
    - `/sessions/session-1/messages/?limit=10`
      -> `{ sessionId: "session-1", action: "messages" }`
    - `/sessions/session-1/#summary`
      -> `{ sessionId: "session-1", action: undefined }`.
...
- Expanded server-route classifier unit coverage for combined
  trailing-slash + hash route variants across core routes and scoped API edge
  forms.
- Updated:
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added assertions for:
    ...
    - unsupported methods on:
      - `POST /health/#summary`
      - `GET /sessions/session-1/prompt/#latest`
      - `POST /sessions/session-1/messages/#tail`
      each -> `METHOD_NOT_ALLOWED` + `CORE_ROUTE_CLASSIFIER`
    - `GET /unknown/#summary` -> `UNHANDLED` + `CORE_ROUTE_CLASSIFIER`
    - missing-action:
      - `GET /sessions/session-1/#latest`
      - `POST /sessions/session-1/#latest`
      each -> `UNHANDLED` + `CORE_ROUTE_CLASSIFIER`
    - API-scope edge forms:
      - `GET /api/#summary`
      - `POST /api//config/#summary`
      each -> `UNHANDLED` + `API_ROUTE_CLASSIFIER`.
...
- Expanded server-route classifier unit coverage for API combined
  trailing-slash + hash forms.
- Updated:
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added assertions for:
    - `GET /api/config/#summary` -> `API_MATCH`
    - `POST /api/config/#summary` -> `METHOD_NOT_ALLOWED` +
...
## 2026-02-14 (B142 api-route root trailing-query not-found coverage)

### Summary
- Expanded api-routes unit coverage for direct classifier handling of API-root
  trailing-query input.
- Updated:
  - `__tests__/unit/server/api-routes.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added assertion for:
    - `classifyApiRoute("GET", "/api/?scope=all")`
    expecting canonical:
    - `NOT_FOUND`
    - `classifierHandler: API_ROUTE_CLASSIFIER`.
...
## 2026-02-14 (B141 core-route health combined-suffix coverage)

### Summary
- Expanded core-route classifier unit coverage for the combined
  trailing-slash + query health route form.
- Updated:
  - `__tests__/unit/server/core-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
...
- Expanded server-route classifier unit coverage for combined trailing-slash +
  query variants across core-route method guards and unknown-core unhandled
  classification.
- Updated:
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added assertions for:
    - `GET /health/?check=true` -> `HEALTH_OK`
    - `POST /health/?check=true` -> `METHOD_NOT_ALLOWED` + core handler
    - `GET /sessions/?scope=all` -> `METHOD_NOT_ALLOWED` + core handler
    - `GET /sessions/:id/prompt/?scope=all` -> `METHOD_NOT_ALLOWED` + core handler
    - `POST /sessions/:id/messages/?scope=all` -> `METHOD_NOT_ALLOWED` + core handler
    - `GET /unknown/?scope=all` -> `UNHANDLED` + core handler.
...
- Changes:
  - added assertions for:
    - method-not-allowed: `POST /health/?check=true`
    - method-not-allowed: `GET /sessions/?scope=all`
    - method-not-allowed: `GET /sessions/:id/prompt/?view=full`
    - method-not-allowed: `POST /sessions/:id/messages/?limit=10`
    - unhandled: `GET /sessions/:id/?view=full`
  - each assertion locks canonical core-route classifier decisions under
    combined normalized path forms.
...
- Expanded password-protected API auth-before-method integration coverage for
  `/api/tui/execute-command` unsupported-method path variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in `applies auth checks before method-not-allowed semantics on api routes`:
    ...
    - added authenticated assertions for the same variants, locking canonical
      `405 + METHOD_NOT_ALLOWED`.
...
## 2026-02-14 (B133 non-api method query-trailing coverage)

### Summary
- Expanded non-API method-not-allowed integration coverage for combined
  trailing-slash + query variants across known non-API routes with unsupported
  methods.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in `returns method not allowed for known non-api routes with unsupported methods`:
    - added assertions for:
      - `/health?probe=1` (POST)
      - `/health/?probe=1` (POST)
      - `/sessions?scope=all`
      ...
      - `/sessions/session-1/prompt/?scope=all`
      - `/sessions/session-1/messages?scope=all` (POST)
      - `/sessions/session-1/messages/?scope=all` (POST)
...
### Summary
- Expanded password-protected health-route auth-bypass integration coverage
  for query and trailing-slash + query route variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in `keeps health-route auth bypass semantics under password protection`:
    - added GET assertions for:
      - `/health?probe=1`
      - `/health/?probe=1`
      each expecting `200` + `{ status: "ok" }`.
    - added POST assertions for:
      - `/health?probe=1`
      - `/health/?probe=1`
      each expecting `405 + METHOD_NOT_ALLOWED` with
      `www-authenticate` header remaining `null`.
...
## 2026-02-14 (B129 non-api query-trailing auth-order coverage)

### Summary
- Expanded password-protected non-api auth-before-method integration coverage
  for combined trailing-slash + query variants of session method-guarded
  routes.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
...
### Summary
- Expanded password-protected unknown-route auth-order integration coverage
  for query-suffixed API/session unknown-route variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in `applies auth checks before not-found semantics on unknown routes`:
    - added unauthenticated assertions for:
      ...
      - `/sessions//messages?tail=1`
      each expecting `401` + `WWW-Authenticate: Bearer`.
    - added authenticated assertions for the same query-suffixed variants,
      locking canonical:
      - `404 + NOT_FOUND` for API variants
      - `404 + UNKNOWN_ENDPOINT` for session variants.
...
## 2026-02-14 (B126 unknown-route trailing auth-order coverage)

### Summary
- Expanded password-protected unknown-route auth-order integration coverage
  for trailing-slash variants across API/core/session malformed paths.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in `applies auth checks before not-found semantics on unknown routes`:
    - added unauthenticated assertions for trailing variants:
      - `/api/`
      - `/unknown-endpoint/`
      - `/sessions/session-1/unsupported/`
      - `/sessions/session-1/`
      - `/sessions//prompt/`
      - `/sessions//messages/`
      expecting `401` + `WWW-Authenticate: Bearer`.
    - added authenticated assertions for the same trailing variants, locking
      canonical:
      - `404 + NOT_FOUND` for API/core malformed API paths
      - `404 + UNKNOWN_ENDPOINT` for session malformed paths.
...
### Summary
- Expanded password-protected non-api auth-before-method integration coverage
  for unauthenticated trailing-slash route variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in `applies auth checks before method semantics on non-api protected routes`:
    - added unauthenticated assertions for:
      ...
      - `GET /sessions/:id/prompt/`
      - `POST /sessions/:id/messages/`
    - each now explicitly asserts `401` with `WWW-Authenticate: Bearer`.
  - also added authenticated trailing `/sessions/` assertion for canonical
    `405` + `METHOD_NOT_ALLOWED`.
...
## 2026-02-14 (B123 non-api auth-order method coverage expansion)
...
- Changes:
  - in `applies auth checks before method semantics on non-api protected routes`:
    - added unauthenticated assertions for:
      - `GET /sessions/session-1/prompt`
      - `POST /sessions/session-1/messages`
      expecting `401` + `WWW-Authenticate: Bearer`.
    - added authenticated assertions for trailing-slash variants:
      - `GET /sessions/session-1/prompt/`
      - `POST /sessions/session-1/messages/`
      expecting `405` + `METHOD_NOT_ALLOWED`.
...
## 2026-02-14 (B122 unknown-route auth-order malformed session coverage)

### Summary
- Expanded password-protected unknown-route auth-order integration coverage
  for malformed session path variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in `applies auth checks before not-found semantics on unknown routes`:
    - added unauthenticated assertions for:
      ...
      each expecting `401` with `WWW-Authenticate: Bearer`.
    - added authenticated assertions for the same routes expecting:
...
## 2026-02-14 (B121 unknown-route auth-order malformed API coverage)

### Summary
- Expanded password-protected unknown-route auth-order integration coverage
  for malformed API path variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - in `applies auth checks before not-found semantics on unknown routes`:
    - added unauthenticated `POST` assertions for:
      - `/api//config`
      - `/api/sessions//messages`
      expecting `401` + `WWW-Authenticate: Bearer`.
    - added authenticated assertions for the same paths expecting canonical
      `404` + `NOT_FOUND`.
...
## 2026-02-14 (B120 core-route missing-action normalization coverage)

### Summary
- Expanded `classifyCoreRoute(...)` coverage for missing-action session route
  variants under normalized path forms.
- Updated:
  - `__tests__/unit/server/core-route-classifier.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - augmented unhandled-route assertions for:
    - `/sessions/session-1/`
    - `/sessions/session-1?view=full`
    - `/sessions/session-1#latest`
  - each now explicitly locks `CORE_ROUTE_DECISION.UNHANDLED`.
...
### Summary
- Expanded integration coverage to lock method-not-allowed semantics for
  trailing-slash API route variants.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - added assertions proving unsupported methods on:
    - `/api/config/`
    - `/api/tui/execute-command/`
...
### Summary
- Added integration assertions to lock auth ordering ahead of not-found
  semantics for unknown API/core routes when password auth is enabled.
- Updated:
  - `__tests__/integration/server/headless-server.integration.test.ts`
  - `PLAN3.md`
- Changes:
  - new test verifies unauthenticated requests to `/api` and
    `/unknown-endpoint` return `401` + auth challenge.
  - same endpoints with valid auth continue to return canonical
    `404` + `NOT_FOUND`.
...
### Summary
- Extracted shared route pathname normalization so classifier/parser entrypoints
  consistently trim input and drop query/hash suffixes for direct-call
  robustness.
- Updated:
  - `src/server/pathname-normalization.ts` (new)
  - `src/server/core-route-classifier.ts`
  - `src/server/api-routes.ts`
  - `src/server/server-route-classifier.ts`
  - `src/server/session-route-path.ts`
  - `__tests__/unit/server/pathname-normalization.unit.test.ts` (new)
  - `__tests__/unit/server/core-route-classifier.unit.test.ts`
  - `__tests__/unit/server/api-routes.unit.test.ts`
  - `__tests__/unit/server/server-route-classifier.unit.test.ts`
  - `__tests__/unit/server/session-route-path.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added `normalizeRoutePathname(...)` helper.
  - classifier and session-path parsing entrypoints now use shared helper.
  - added regression coverage for query/hash suffixed route-path inputs across
    helper/core/API/server/session route parsing behavior.
...
### Summary
- Added post-completion hardening for standardized request parsing telemetry
  when method input is blank/whitespace.
- Updated:
  - `src/server/request-error-normalization.ts`
  - `__tests__/unit/server/request-error-normalization.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - parsing/validation log context normalization now falls back blank methods
    to `UNKNOWN` for canonical telemetry payloads.
  - added focused unit coverage asserting method fallback behavior.
...
## 2026-02-14 (B72 request parsing-log pathname fallback hardening)

### Summary
- Added post-completion hardening for standardized request parsing telemetry
  when route path input is blank/whitespace.
- Updated:
  - `src/server/request-error-normalization.ts`
  - `__tests__/unit/server/request-error-normalization.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - parsing/validation log context normalization now falls back blank pathnames
    to `/` for canonical telemetry payloads.
  - added focused unit coverage asserting root-path fallback behavior.
...
### Summary
- Added post-completion coverage hardening for NutJS early gate outcomes and
  feature-flag short-circuit behavior.
- Updated:
  - `__tests__/unit/utils/nutjs-execution-gate.utils.unit.test.ts`
  - `__tests__/e2e/skippable.nutjs-smoke.e2e.test.ts`
  - `PLAN3.md`
- Changes:
  - added unit assertions that `disabled` and `not_allowlisted` outcomes
    return no capability/diagnostics metadata.
  - added smoke test for feature-flag-off path confirming disabled outcome and
    no action invocation.
...
## 2026-02-14 (B66 unsupported-platform diagnostics hardening)

### Summary
- Added post-completion diagnostics hardening for unsupported NutJS platform
  simulation paths.
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - added diagnostics helper unit coverage asserting unsupported platforms
    remain `not_applicable` and do not trigger missing-permission checks.
  - added smoke simulation for unsupported platform (`aix`) and asserted
    capability-noop outcome with fully non-applicable diagnostics statuses.
...
## 2026-02-14 (B65 no-op diagnostics + null-result smoke coverage hardening)

### Summary
- Added post-completion test hardening for NutJS capability no-op diagnostics
  completeness and executed-null smoke behavior.
- Updated:
  - `__tests__/unit/utils/nutjs-execution-gate.utils.unit.test.ts`
  - `__tests__/e2e/skippable.nutjs-smoke.e2e.test.ts`
  - `PLAN3.md`
- Changes:
  - expanded unsupported-platform capability-noop assertions to verify all
    diagnostics statuses are `not_applicable`.
  - added smoke assertion that allowlisted runtime-enabled actions resolving
    `null` preserve `executed` outcomes.
...
### Summary
- Added post-completion hardening for wildcard precedence behavior in normalized
  allowlist policies.
- Updated:
  - `src/utils/nutjs-execution-gate.utils.ts`
  - `__tests__/unit/utils/nutjs-execution-gate.utils.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - normalization now collapses any wildcard-containing allowlist to `["*"]`.
  - kept explicit dedupe-order tests and added wildcard collapse assertion.
...
## 2026-02-14 (B59 allowlist deduplication hardening)

### Summary
- Added post-completion hardening for normalized allowlist output stability.
- Updated:
  - `src/utils/nutjs-execution-gate.utils.ts`
  - `__tests__/unit/utils/nutjs-execution-gate.utils.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - allowlist normalization now deduplicates repeated entries while preserving
    first-seen ordering.
  - added unit coverage for deduplicating duplicate explicit entries and
    wildcard entries under mixed-case input.
...
### Summary
- Added post-completion smoke-suite hardening for cross-platform permission
  diagnostics simulations.
- Updated:
  - `__tests__/e2e/skippable.nutjs-smoke.e2e.test.ts`
  - `PLAN3.md`
- Changes:
  - added macOS accessibility-denied smoke simulation with expected
    `permission_missing`.
  - added Windows low-integrity smoke simulation with expected
    `permission_missing`.
  - asserted diagnostics status metadata for both new smoke simulations.

### Validation
- Targeted:
  - `npx vitest run __tests__/e2e/skippable.nutjs-smoke.e2e.test.ts` ✅
- Full gates (equivalent commands; bun/bunx unavailable in this shell):
  - `bun run lint` ❌ (`bun: command not found`)
  - `bun run typecheck` ❌ (`bun: command not found`)
  - `bun run test` ❌ (`bun: command not found`)
...
## 2026-02-14 (B54 cross-platform permission-gate assertions)

### Summary
- Added post-completion cross-platform hardening for NutJS permission diagnostics
  gate semantics.
- Updated:
  - `__tests__/unit/utils/nutjs-execution-gate.utils.unit.test.ts`
  - `PLAN3.md`
- Changes:
  - added macOS accessibility-denied assertion path expecting
    `permission_missing`.
  - added Windows low-integrity assertion path expecting
    `permission_missing`.
  - asserted diagnostics status payloads for both new cross-platform paths.
...
### Summary
- Added focused post-completion smoke hardening for Linux headless permission
  behavior in NutJS CI coverage.
- Updated:
  - `__tests__/e2e/skippable.nutjs-smoke.e2e.test.ts`
  - `PLAN3.md`
- Changes:
  - added Linux-only smoke assertion for runtime-enabled NutJS gate with no
    display backend, expecting deterministic `permission_missing`.
  - preserved cross-platform smoke behavior by gating assertion to Linux only.
...
## 2026-02-14 (B51 permission-aware NutJS gate integration)

### Summary
- Added a post-completion hardening increment to align NutJS runtime behavior
  with documented fallback precedence ordering.
- Updated:
  ...
  - `PLAN3.md`
- Changes:
  - introduced `permission_missing` execution outcome.
  - NutJS gate now evaluates permission diagnostics before action execution and
    blocks only on explicit missing-permission diagnostics.
  - unknown/not-applicable diagnostics remain non-blocking.
  - expanded unit and e2e coverage for permission-missing and unknown-state
    execution outcomes.
...
### Summary
- Completed P2 backlog item B42 in `PLAN3.md` by deduplicating shell invocation
  setup logic across shell-session, interactive-shell, and background tasks.
- Updated:
...
- Changes:
  - added shared shell invocation utility to centralize env snapshot retrieval
    and platform shell invocation spec creation.
  - replaced repeated invocation setup logic across the three shell execution
    paths with shared helper usage.
  - added focused invocation utility tests for Windows and POSIX behavior.

### Validation
- Targeted:
  - `npx vitest run __tests__/unit/utils/shell-invocation.utils.unit.test.ts __tests__/unit/utils/platform-shell.utils.unit.test.ts __tests__/unit/tools/shell-session.unit.test.ts` ✅
- Full gates (equivalent commands; bun/bunx unavailable in this shell):
  - `bun run lint` ❌ (`bun: command not found`)
  - `bun run typecheck` ❌ (`bun: command not found`)
  - `bun run test` ❌ (`bun: command not found`)
...
### Summary
- Completed P1 backlog item B36 in `PLAN3.md` by adding per-runtime TTL caching
  for update-check outcomes.
- Updated:
  - `src/config/limits.ts`
  - `src/utils/update-check.ts`
  - `__tests__/unit/utils/update-check.unit.test.ts`
- Changes:
  - introduced runtime update-check TTL cache state that memoizes recent check
    outcomes (including failed fetch attempts).
  - ensured test reset utility clears runtime cache state.
  - added focused unit coverage validating repeated check dedupe behavior.

### Validation
- Targeted:
  - `npx vitest run __tests__/unit/utils/update-check.unit.test.ts` ✅
- Full gates (equivalent commands; bun/bunx unavailable in this shell):
  - `bun run lint` ❌ (`bun: command not found`)
  - `bun run typecheck` ❌ (`bun: command not found`)
  - `bun run test` ❌ (`bun: command not found`)
...
### Summary
- Completed P1 backlog item B34 in `PLAN3.md` by shifting session export writes
  to streamed pipelines for large payload safety.
- Updated:
  - `src/utils/session-export.ts`
  - `__tests__/unit/utils/session-export.unit.test.ts`
- Changes:
  - JSON and Markdown exports now write chunked stream output directly to disk.
  - ZIP export writeout now streams generated output into file stream.
  - expanded unit coverage to validate JSON, Markdown, and ZIP export/import
    flows.

### Validation
- Targeted:
  - `npx vitest run __tests__/unit/utils/session-export.unit.test.ts` ✅
- Full gates (equivalent commands; bun/bunx unavailable in this shell):
  - `bun run lint` ❌ (`bun: command not found`)
  - `bun run typecheck` ❌ (`bun: command not found`)
  - `bun run test` ❌ (`bun: command not found`)
